...
public class ProfessorView extends JDialog implements ActionListener {
	...
	public ProfessorView(LoginView loginView, Professor loggedProf) {
		...
		try {
			trainers = getTrainersAndConstructComboBoxes();
		} catch (MyException er) {
			JOptionPane.showMessageDialog(null, er.getMessage(), "WARNING", JOptionPane.ERROR_MESSAGE);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(deleteButton)) {
			if (JOptionPane.showOptionDialog(panelBattle, "Do you want to delete this trainner?", "Confirm delete",
				...
				try {
					...
					try {
						trainers = getNewTrainersAndReSetComboBoxes();
		...
		if (e.getSource().equals(ascentTrainerCB)) {
			...
			//I dont know why, but everytime you delete a trainer, this  action performed method triggers, so as much as I would want
			//to show a message, it would always trigger and make no sense. You can try if you want , the  popup window is below to 
			//uncomment if you want to try

            // JOptionPane.showMessageDialog(null, "No trainer selected to ascend" "WARNING", JOptionPane.ERROR_MESSAGE);
		}
	...
	private LinkedHashSet<Trainer> getTrainersAndConstructComboBoxes() throws MyException {
		LinkedHashSet<Trainer> tSet = manageable.getTrainers();
	...
	private void updateTrainersAndComboBoxes(String name) throws MyException {
